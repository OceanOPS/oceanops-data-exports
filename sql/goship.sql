-- Layer: goship
-- GO-SHIP design lines — core line list + sampled since edition date (cruise departure)
-- Edit filter under @where; edition.values.json for dates / line lists.

-- @where
g.shape IS NOT NULL AND g.name IN ({{GOSHIP_LINE_NAMES_IN}}) AND EXISTS (
  SELECT 1
  FROM oceanops.cruise_line cl
  JOIN oceanops.cruise c ON c.id = cl.cruise_id
  WHERE cl.line_id = g.line_id
    AND c.departure_date >= DATE '{{GOSHIP_SAMPLED_SINCE}}'
)
