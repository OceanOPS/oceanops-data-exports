-- Layer: anibos
-- AniBOS — layer-table statuses, ANIMAL family or AniBOS network, latest_loc_date in last 12 months
...
-- @where
t.ptf_status IN ({{LAYER_TABLE_PTF_STATUS_IN}}) AND (t.ptf_family = 'ANIMAL' OR t.network ILIKE '%AniBOS%') AND t.latest_loc_date >= DATE '{{ROLLING_12M_SINCE}}'
