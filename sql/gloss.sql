...
--   psql "$OCEANOPS_DATABASE_URL" -v ON_ERROR_STOP=1 -f sql/gloss.sql
-- Edit filter under @where; edition.values.json for dates / line lists.
-- pgAdmin: npm run render:sql -- sql/gloss.sql

-- @where
t.ptf_status = {{PTF_STATUS_OPERATIONAL}} AND t.network LIKE '%GLOSS%'
