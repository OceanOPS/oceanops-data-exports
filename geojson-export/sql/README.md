# GeoJSON SQL — one file per network / map layer
...
2. **`geojson-export/sql/<layerId>.sql`** — edit `-- @where` (and query shape if needed).
3. Render for pgAdmin:

   ```bash
   npm run render:sql -- geojson-export/sql/argo.sql   # full file, tokens applied
...
Copy from `_template_point.sql` or `_template_line.sql` when adding a layer; register in `../layers.manifest.json`.
